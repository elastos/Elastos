...
# Set ANDROID_NDK to you NDK location. For example,
# /opt/android-ndk-r8e or /opt/android-ndk-r9. This can be done in a
# login script. If ANDROID_NDK is not specified, the script will
# try to pick it up with the value of _ANDROID_NDK below. If
...
# We expect something like ANDROID_NDK=/opt/android-ndk-r8e
# or ANDROID_NDK=/usr/local/android-ndk-r8e.

if [ -z "$ANDROID_NDK" ]; then

  _ANDROID_NDK=""
  if [ -z "$_ANDROID_NDK" ] && [ -d "/usr/local/$_ANDROID_NDK" ]; then
    _ANDROID_NDK="/usr/local/$_ANDROID_NDK"
  fi

  if [ -z "$_ANDROID_NDK" ] && [ -d "/opt/$_ANDROID_NDK" ]; then
    _ANDROID_NDK="/opt/$_ANDROID_NDK"
  fi

  if [ -z "$_ANDROID_NDK" ] && [ -d "$HOME/$_ANDROID_NDK" ]; then
    _ANDROID_NDK="$HOME/$_ANDROID_NDK"
  fi

  if [ -z "$_ANDROID_NDK" ] && [ -d "$PWD/$_ANDROID_NDK" ]; then
    _ANDROID_NDK="$PWD/$_ANDROID_NDK"
  fi

  # If a path was set, then export it
  if [ ! -z "$_ANDROID_NDK" ] && [ -d "$_ANDROID_NDK" ]; then
    export ANDROID_NDK="$_ANDROID_NDK"
  fi
fi

# Error checking
# ANDROID_NDK should always be set by the user (even when not running this script)
# http://groups.google.com/group/android-ndk/browse_thread/thread/a998e139aca71d77
if [ -z "$ANDROID_NDK" ] || [ ! -d "$ANDROID_NDK" ]; then
  echo "Error: ANDROID_NDK is not a valid path. Please edit this script."
  # echo "$ANDROID_NDK"
  # exit 1
fi
...
if [ ! -d "$ANDROID_NDK/toolchains" ]; then
  echo "Error: ANDROID_NDK/toolchains is not a valid path. Please edit this script."
  # echo "$ANDROID_NDK/toolchains"
  # exit 1
fi

# Error checking
if [ ! -d "$ANDROID_NDK/toolchains/$_ANDROID_EABI" ]; then
  echo "Error: ANDROID_EABI is not a valid path. Please edit this script."
  # echo "$ANDROID_NDK/toolchains/$_ANDROID_EABI"
  # exit 1
fi

#####################################################################

# Based on ANDROID_NDK, try and pick up the required toolchain. We expect something like:
