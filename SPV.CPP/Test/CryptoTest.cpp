...
TEST_CASE("Encrypt and decrypt", "[Crypto]") {
	...
	SECTION("decrypt and encrypt") {
		...
		REQUIRE(Crypto::Decrypt(plaintext, ct, passwd, salt, iv, adata, ks == 128 ? true : false));
		...
		REQUIRE(!Crypto::Decrypt(plaintext, ct, wrongPasswd, salt, iv, adata, ks == 128 ? true : false));
		REQUIRE(plaintext != plainResult);
