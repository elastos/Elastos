...
TEST_CASE( "ChainParams construct test", "[ChainParams]" ) {
	...
	SECTION("Construct with BRChainParams") {
		...
		REQUIRE(cp.getRaw()->dnsSeeds[0] == BRMainNetDNSSeeds[0]);
		REQUIRE(cp.getRaw()->dnsSeeds[1] == BRMainNetDNSSeeds[1]);
		REQUIRE(cp.getRaw()->dnsSeeds[2] == BRMainNetDNSSeeds[2]);
		REQUIRE(cp.getRaw()->dnsSeeds[3] == BRMainNetDNSSeeds[3]);
	}
}
