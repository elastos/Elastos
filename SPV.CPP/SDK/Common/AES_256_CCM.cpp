...
namespace Elastos {
	namespace ElaWallet {
		...
		bool AES_256_CCM::EncryptCCM(CMBlock &ct, CMBlock &tag, const CMBlock &plainText, const CMBlock &adata,
			...
			if (1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_CCM_SET_IVLEN, 15 - ivLen, NULL)) {
				Log::getLogger()->error("Encrypt: Set IV len to {}", 15 - ivLen);
				EVP_CIPHER_CTX_free(ctx);
				return false;
			}
		...
		bool AES_256_CCM::DecryptCCM(CMBlock &plainText, const CMBlock &ct, const CMBlock &tag, const CMBlock &adata,
									const CMBlock &key, const CMBlock &iv, bool AES128) {
			EVP_CIPHER_CTX *ctx;
			int len;
			...
			if (1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_CCM_SET_IVLEN, 15 - ivLen, NULL)) {
				Log::getLogger()->error("Decrypt Setting IV len to {}", 15 - ivLen);
				EVP_CIPHER_CTX_free(ctx);
				return false;
			}
		...
		void AES_256_CCM::GenerateSaltAndIV(std::string &saltBase64, std::string &ivBase64) {
		...
		std::string AES_256_CCM::Base64Encode(const CMBlock &buffer) {
			BIO *bio, *b64;
			BUF_MEM *bufferPtr;
		...
		bool AES_256_CCM::Encrypt(std::string &ctBase64, const std::string &plainText, const std::string &passwd,
		...
		bool AES_256_CCM::Encrypt(std::string &ctBase64, const CMBlock &plainText, const std::string &passwd,
								  const std::string &saltBase64, const std::string &ivBase64,
								  const std::string &adataBase64, bool AES128) {
		...
		size_t AES_256_CCM::CalcDecodeLength(const std::string &b64input) {
		...
		CMBlock AES_256_CCM::Base64Decode(const std::string &base64Message) {
			BIO *bio, *b64;
		...
		bool AES_256_CCM::Decrypt(std::string &plainText, const std::string &ctBase64, const std::string &passwd,
								  const std::string &saltBase64, const std::string &ivBase64,
								  const std::string &adataBase64, bool AES128) {
			CMBlock plainData;
		...
		bool AES_256_CCM::Decrypt(CMBlock &plainText, const std::string &ctBase64, const std::string &passwd,
								  const std::string &saltBase64, const std::string &ivBase64,
								  const std::string &adataBase64, bool AES128) {
			...
			if (ct.GetSize() == 0 || 8 > ct.GetSize()) {
				Log::getLogger()->error("ct length {} too short", ct.GetSize());
				return false;
			}
