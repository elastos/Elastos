...
#define TURN_SERVER_REALM_SUFFIX        "auth.tox"
...
int ela_get_turn_server(ElaCarrier *w, ElaTurnServer *turn_server)
{
    ...
    text_len = sizeof(turn_server->realm);
    base58_encode(nonce, sizeof(nonce), turn_server->realm, &text_len);
    strcat(turn_server->realm, "."TURN_SERVER_REALM_SUFFIX);

    strcpy(turn_server->username, w->me.userid);
