    ...
    render () {
        return (
            <div className="p_developerLearnDetail">
                ...
                <ul>
                    <li>
                        <strong>Whisper</strong>: Represents a carrier node object. It is used to control the initialization, start and stop of nodes
                    </li>
                    <li>
                        <strong>Friend</strong>: A friend, represents a node to license another node. For example, if one node sends data to another node, it must first establish a friend relationship. Otherwise, they cannot communicate. For friends, you can add, delete, send messages, etc. A friend-based message communication is a limited-length, unreliable communication mechanism similar to a UDP packet.
                    </li>
                    <li>
                        <strong>Session</strong>: If you want to establish a long connection with high time efficienty and high reliability data transmission channel between the two nodes, you can choose to use the session mode. Initialization, requests and confirmation requests, etc are available for a session.
                    </li>
                    <li>
                        <strong>Stream</strong>: After the session is successfully established, the nodes transfer data by reading and writing from and to a stream.
                    </li>
                    <li>
                        <strong>Channel</strong>: This is a multiplexing implementation for the same session which is a wrapper around the use of a session.
                    </li>
                </ul>
