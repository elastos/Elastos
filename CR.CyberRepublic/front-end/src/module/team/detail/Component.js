...
import {
  message, Col, Row, Tag, Carousel, Avatar, Button, Spin,
  Table, Input, Form, Divider, Popconfirm, Modal
} from 'antd'
...
:
      ...
      return (
        <div className="text-right">
          ...
          {canWithdraw(candidate._id) && (
            <span>
              <a
                onClick={this.withdrawUser.bind(this, candidate._id)}>
  ...
  getApplicationForm() {
    ...
    return (
      <Form onSubmit={this.handleSubmit.bind(this)} className="application-form">
        ...
        <Button loading={this.props.loading} className="d_btn pull-left"
  ...
  async leaveTeam() {
    const member = _.find(this.props.detail.members, (curMember) => {
      return curMember.user._id === this.props.currentUserId &&
  ...
  getMainActions() {
    ...
    const mainActionButton = isTeamMember
      ...
      : (
        <Button disabled={true} type="primary" onClick={() => this.setState({applying: true})}>
          {hasApplied
