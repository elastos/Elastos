...
const stickyEmail = false
const topDif = 0
const stickyBreak = 1024
const stickyH = 77
let lastSt = 0
let autoOff = false
...
function setSticky() {
  ...
  if (sT < lastSt && winW >= stickyBreak) {
    if (!stickyOpen && sT > 110) {
      ...
      TweenMax.to($('.sticky'), 0.75, {delay: 0.2, transform: `translate3d(0px, ${0}px, 0px)`, display: 'block', ease: Power3.easeOut})
      stickyOpen = true
    }
  ...
  if (sT > lastSt) {
    if (stickyOpen) {
      $('.sticky').removeClass('open')
      TweenMax.to($('.sticky'), 0.75, {transform: `translate3d(0px, ${-stickyH}px, 0px)`, ease: Power3.easeInOut})
      stickyOpen = false
    }
  }
  ...
  if (sT <= 0 && !autoOff) {
    ...
    autoOff = true
    TweenMax.to($('.sticky'), 0.75, {transform: `translate3d(0px, ${-stickyH}px, 0px)`,
      display: 'none',
      ease: Power3.easeOut,
      onComplete() {
        autoOff = false
...
const loadSp = 0.7
const loadDel = 0.25
const topDel = loadDel * 6
// var endDel = ((topDel*2)*1000) - 200;
const endDel = 2
const loadEase = 'Power3.easeInOut'

freezePage()

function animateLogo() {
  TweenMax.to($('.logo-mark>img'), 0.5, {scaleX: 1, scaleY: 1, opacity: 1, ease: Power3.easeOut})
...
var loadChecker = setInterval(() => {
  ...
  if (loadReady) {
    ...
    // slide over
    TweenMax.to('.load-clip', 0.75, {delay: 0.5, x: 0, ease: Power3.easeInOut})
    TweenMax.to('.logo-text', 0.5, {delay: 0.7, width: '259px', opacity: 1, ease: Power3.easeInOut})

    // reveal page
    loadSC = 0.6; if (winW <= 650) {loadSC = 0.4}
    TweenMax.to('.load-clip', 0.5, {delay: 1.5, opacity: 0, scaleX: loadSC, scaleY: loadSC, ease: Power3.easeInOut})
    TweenMax.to('#loader', 0.75, {delay: 1.7,
      opacity: 0,
      display: 'none',
      onComplete() {
        unfreezePage()
...
function openVideoOverlay() {
  ...
  lb_video.addEventListener('loadedmetadata', () => {
    lb_video.play()
  }, false)
}
...
function closeVideoOverlay() {
  unfreezePage()
  TweenMax.to($('#video-overlay'), 0.5, {opacity: 0, display: 'none'})
  lb_video.pause()
}
...
const tl1 = new TimelineMax({repeat: -1, onRepeat: fireRadio1})
let motionPath1
...
const tl2 = new TimelineMax({repeat: -1, onRepeat: fireRadio2})
let motionPath2
...
let modelCur = 0
let nextCur = 0
const totModelSlides = $('.model-slide').length
let modelRow = 3
let totModelGroups = Math.ceil(totModelSlides / modelRow)
let slideWinW = 1200
let modelPer = 0.3
...
const app_offsets = [0, 1, 1.6, 0.3, 0.8, 1.2]
...
function offsetPings(elem, offset) {
  ...
  elem.find('.radio-group').each(function(i) {
    rgap = radioGap
    if ($(this).hasClass('light')) {rgap = 0.75}
...
function initScrollMagic() {
  ...
  $('.hasAnim').each(function() {
    const currentElem = `#${$(this).attr('id')}`
    // console.log(currentElem)
    const scene = new ScrollMagic.Scene({triggerElement: currentElem, triggerHook: 2, duration: winH + $(currentElem).outerHeight()})
      .addTo(controller)
  ...
  const pillarsScene = new ScrollMagic.Scene({
  ...
  const appScene = new ScrollMagic.Scene({
