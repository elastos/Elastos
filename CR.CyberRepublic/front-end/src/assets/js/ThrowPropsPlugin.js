...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(() => {
  ...
  _gsScope._gsDefine('plugins.ThrowPropsPlugin', ['plugins.TweenPlugin', 'TweenLite', 'easing.Ease', 'utils.VelocityTracker'], (TweenPlugin, TweenLite, Ease, VelocityTracker) => {
    ...
    let ThrowPropsPlugin = function(props, priority) {
    ...
    ThrowPropsPlugin._cssRegister = function() {
      ...
      let _internals = CSSPlugin._internals,
 ...
 */
  _gsScope._gsDefine('utils.VelocityTracker', ['TweenLite'], (TweenLite) => {
    ...
    let _first,	_initted, _time1, _time2,
      _capsExp = /([A-Z])/g,
      _empty = {},
      ...
      VelocityTracker = function(target) {
        this._lookup = {}
        this.target = target
        this.elem = !!((target.style && target.nodeType))
        if (!_initted) {
