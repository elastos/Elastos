...
class HealthServicer(health_check_pb2_grpc.HealthServicer):
    ...
    def Watch(self, request, context, send_response_callback=None):
        ...
        with self._lock:
            ...
            if status is None:
                status = health_check_pb2.HealthCheckResponse.SERVICE_UNKNOWN  # pylint: disable=no-member
            send_response_callback(
                health_check_pb2.HealthCheckResponse(status=status))
        ...
        with self._lock:
            if self._gracefully_shutting_down:
                ...
                for service in self._server_status:
                    self.set(service,
                             health_check_pb2.HealthCheckResponse.NOT_SERVING)  # pylint: disable=no-member
