...
class RateLimiter:
	...
	def get_last_access_count(self, api_key, service_name):
		...
		result = self.session.query(ServicesLists).filter_by(service_name=service_name, user_api_id=api_key_data.id).first()
		
		if result is not None:
			date1 = datetime.datetime.now().strftime(self.date_format)
			date2 = result.last_access.strftime(self.date_format)
			diff = datetime.datetime.strptime(date1, self.date_format) - datetime.datetime.strptime(date2, self.date_format)
	...
	def add_access_count(self, user_api_id, service_name, flag):
		service_list_data = self.session.query(ServicesLists).filter_by(service_name=service_name, user_api_id=user_api_id).first()
