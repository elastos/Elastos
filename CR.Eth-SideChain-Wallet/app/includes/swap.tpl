...
<main class="tab-pane swap-tab active" ng-if="globalService.currentTab==globalService.tabs.swap.id"
    ...
    <section class="bity-contact text-center">
        <p><a class="btn-warning btn-sm"
              href="mailto:support@mycrypto.com?Subject={{orderResult.orderId}}%20Issue%20regarding%20my%20Swap%20via%20MyCrypto%20&Body=%0APlease%20include%20the%20below%20if%20this%20issue%20is%20regarding%20your%20order.%20%0A%0AREF%20ID%23%3A%20{{orderResult.orderId}}%0A%0AAmount%20to%20send%3A%20{{orderResult.depositAmount}}%20{{orderResult.inputCurrency}}%0A%0AAmount%20to%20receive%3A%20{{orderResult.withdrawalAmount}}%20{{orderResult.pair.split('_')[1].toUpperCase()}}%0A%0APayment%20Address%3A%20{{orderResult.deposit}}%0A%0ARate%3A%20{{swapOrder.swapRate}}%20{{swapOrder.swapPair}}%0A%0A"
