...
The code tutorial [p2p programming in Elastos.ELA.SideChain.ETH](https://github.com/nolash/Elastos.ELA.SideChain.ETH-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `Elastos.ELA.SideChain.ETH` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `Elastos.ELA.SideChain.ETH` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
