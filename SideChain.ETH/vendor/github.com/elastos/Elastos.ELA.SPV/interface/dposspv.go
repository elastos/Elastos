...
func NewDPOSSPVService(cfg *DPOSConfig, interrupt <-chan struct{}) (DPOSSPVService, error) {
	...
	getBlock := func(h *util.Header) (block *types.Block, e error) {
		header := h.BlockHeader.(*iutil.DHeader)
		txIDs, err := dposStore.Txs().GetIds(h.Height)
		if err != nil {
			return nil, err
		}
		block = &types.Block{Header: header.Header}
		block.Transactions = make([]*types.Transaction, 0, len(txIDs))
		for _, txID := range txIDs {
			itx, err := dposStore.Txs().Get(txID)
			if err != nil {
				return nil, err
			}
			var tx types.Transaction
			err = tx.Deserialize(bytes.NewReader(itx.RawData))
			if err != nil {
				return nil, err
			}
	...
	arbiters, err := state.NewArbitrators(cfg.ChainParams, nil,
		func() uint32 {
		...
		func() (block *types.Block, e error) {
			...
			return getBlock(best)
		},
		func(height uint32) (block *types.Block, e error) {
			header, err := headerStore.GetByHeight(height)
			if err != nil {
				return nil, err
			}
	...
	serviceCfg := &sdk.Config{
		...
		GenesisHeader:  GenesisDHeader(cfg.ChainParams.GenesisBlock),
