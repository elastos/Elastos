...
import (
	...
	nb "github.com/elastos/Elastos.ELA.SideChain.NeoVM/blockchain"
	nmsg "github.com/elastos/Elastos.ELA.SideChain.NeoVM/p2p/server/msg"
...
func (sp *serverPeer) OnNep5Log(_ *peer.Peer, msg *nmsg.Nep5LogMsg) {
	storedb := nb.DefaultChain.Store

	height := msg.GetHeight()
	hash := msg.GetBlockHash()
	topicAddr := msg.GetTopicAddr()

	var logs  []*ntypes.Nep5Log
	receipts, err := storedb.GetReceipts(height, &hash)
	if err != nil {
		log.Errorf("can not find receipt by blockhash:%s\n", hash.String())
		return
	}
	...
	for _, receipt := range receipts {
		for _, log := range receipt.Logs {
			...
			if log.From.IsEqual(topicAddr) {
				logs = append(logs, log)
			} else if log.To.IsEqual(topicAddr) {
				logs = append(logs, log)
			}
		}
	}

	sp.pushNep5LogResponse(logs)
}

func (sp *serverPeer) pushNep5LogResponse(logs []*ntypes.Nep5Log) {
	resps := make([]ntypes.Nep5Log, 0)
	for _, ll := range logs {
		l := ntypes.Nep5Log{}

		l.Address = ll.Address
		l.From = ll.From
		l.To = ll.To
		l.Data = ll.Data
		l.TxHash = ll.TxHash
...
func (s *Server) handlePeerMsg(peers map[p2psvr.IPeer]*serverPeer, p interface{}) {
	...
	case newPeerMsg:
		...
		sp.Peer.AddMessageFunc(func(_ *elapeer.Peer, m p2p.Message) {
			switch m := m.(type) {
...
func makeEmptyMessage(cmd string) (p2p.Message, error) {
	...
	case p2p.CmdReject:
		message = &msg.Reject{}
	case nmsg.CmdNep5Log:
		message = &nmsg.Nep5LogMsg{}
