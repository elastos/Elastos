...
func NewStackItems() []datatype.StackItem {
	return make([]datatype.StackItem, 0)
}

func NewStackItem(data interface{}) (datatype.StackItem, error) {
	var stackItem datatype.StackItem
	var err error
	switch data.(type) {
	...
	case []byte:
		stackItem = datatype.NewByteArray(data.([]byte))
	case []datatype.StackItem:
		stackItem = datatype.NewArray(data.([]datatype.StackItem))
	case datatype.StackItem:
		stackItem = data.(datatype.StackItem)
	case interfaces.IGeneralInterface:
		stackItem = datatype.NewGeneralInterface(data.(interfaces.IGeneralInterface))
	case *datatype.ByteArray:
		stackItem = data.(*datatype.ByteArray)
	case *datatype.Integer:
		stackItem = data.(*datatype.Integer)
	case *datatype.Array:
		stackItem = data.(*datatype.Array)
	case *datatype.Boolean:
		stackItem = data.(*datatype.Boolean)
	default:
...
func AssertStackItem(stackItem interface{}) datatype.StackItem {
	if s, ok := stackItem.(datatype.StackItem); ok {
		return s
	}
	return nil
}
...
func PopStackItem(e *ExecutionEngine) datatype.StackItem {
	return Pop(e).(datatype.StackItem)
}
...
func PeekStackItem(e *ExecutionEngine) datatype.StackItem  {
	return Peek(e).(datatype.StackItem)
}
...
func PeekNStackItem(i int, e *ExecutionEngine) datatype.StackItem {
	return PeekN(i, e).(datatype.StackItem)
}
