...
func BuildBloomFilter(addresses []*common.Uint168, outpoints []*util.OutPoint) *Filter {
	elements := uint32(len(addresses) + len(outpoints))

	filter := NewFilter(elements, 0, fprate.ReducedFalsePositiveRate)
...
func FilterAddress(filter Filter, address *common.Uint168) {
	filter.Add(address.Bytes())
}

// Add a outpoint into the given bloom filter
func FilterOutpoint(filter Filter, op *util.OutPoint) {
	filter.Add(op.Bytes())
}
...
func merkleParent(left *common.Uint256, right *common.Uint256) (*common.Uint256, error) {
	// dupes can screw things up; CVE-2012-2459. check for them
	if left != nil && right != nil && left.IsEqual(*right) {
		return nil, fmt.Errorf("DUP HASH CRASH")
	}
	// if left child is nil, output nil.  Need this for hard mode.
	if left == nil {
		return nil, fmt.Errorf("Left child is nil")
	}
...
func CheckMerkleBlock(m msg.MerkleBlock) ([]*common.Uint256, error) {
	...
	for {
		...
		if inDeadZone(pos, m.Transactions) {
			...
			h, err := merkleParent(s[tip].h, nil)
			if err != nil {
				return r, err
			}
			s[tip-1].h = h
		...
		if tip > 1 && s[tip-1].h != nil && s[tip].h != nil {
			...
			h, err := merkleParent(s[tip-1].h, s[tip].h)
			if err != nil {
				return r, err
			}
			s[tip-2].h = h
