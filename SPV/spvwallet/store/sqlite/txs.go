...
func (t *txs) Get(txId *common.Uint256) (*sutil.Tx, error) {
	...
	return &sutil.Tx{TxId: *txId, Height: height, Timestamp: time.Unix(timestamp, 0), Data: tx}, nil
}
...
func (t *txs) GetAllFrom(height uint32) ([]*sutil.Tx, error) {
	t.RLock()
	defer t.RUnlock()
	...
	for rows.Next() {
		...
		txns = append(txns, &sutil.Tx{TxId: *txId, Height: height, Timestamp: time.Unix(timestamp, 0), Data: tx})
	}
