...
func (u *utxos) Get(outPoint *core.OutPoint) (*sutil.UTXO, error) {
	u.RLock()
	defer u.RUnlock()

	row := u.QueryRow(`SELECT Value, LockTime, AtHeight, Address FROM UTXOs WHERE OutPoint=?`, outPoint.Bytes())
	var valueBytes []byte
	var lockTime uint32
	var atHeight uint32
	...
	return &sutil.UTXO{Op: *outPoint, Value: *value, LockTime: lockTime, AtHeight: atHeight, Address: *address}, nil
}
...
func (u *utxos) getUTXOs(rows *sql.Rows) ([]*sutil.UTXO, error) {
	...
	for rows.Next() {
		...
		utxo := &sutil.UTXO{Op: *outPoint, Value: *value, LockTime: lockTime, AtHeight: atHeight, Address: *address}
...
func (u *utxos) Del(outPoint *core.OutPoint) error {
	u.Lock()
	defer u.Unlock()

	_, err := u.Exec("DELETE FROM UTXOs WHERE OutPoint=?", outPoint.Bytes())
	return err
}
