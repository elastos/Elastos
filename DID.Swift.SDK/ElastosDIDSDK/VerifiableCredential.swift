...
public class VerifiableCredential: DIDObject {
    ...
    private func traceCheck(_ rule: Int) throws -> Bool {
        let controllerDoc = try getSubject()?.did.resolve()
        ...
        if !isSelfProclaimed() {
            let issuerDoc = try getIssuer()?.resolve()
    ...
    private func checkGenuine() throws -> Bool {
        ...
        return try doc!.makeVerificationWithIdentity(proof.verificationMethod, proof.signature, [data])
    }
