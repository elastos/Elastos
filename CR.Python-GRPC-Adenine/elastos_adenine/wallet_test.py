...
def test_request_ela_did():
	wallet = Wallet(test_input['host'], test_input['port'], test_input['production'])
	response = wallet.request_ela(test_input['api_key_to_use'], test_input['did_to_use'], 'did', test_input['ela_to_use'])
	if response['status']:
		json_output = json.loads(response['output'])
		for i in json_output['result']:
...
def test_request_ela_token():
	wallet = Wallet(test_input['host'], test_input['port'], test_input['production'])
	response = wallet.request_ela(test_input['api_key_to_use'], test_input['did_to_use'], 'token', test_input['ela_to_use'])
	if response['status']:
		json_output = json.loads(response['output'])
		for i in json_output['result']:
...
def test_request_ela_eth():
	wallet = Wallet(test_input['host'], test_input['port'], test_input['production'])
	response = wallet.request_ela(test_input['api_key_to_use'], test_input['did_to_use'], 'eth', test_input['ela_to_use'])
	if response['status']:
		json_output = json.loads(response['output'])
		for i in json_output['result']:
