...
def test_upload_and_sign():
	...
	response = hive.upload_and_sign(test_input['api_key_to_use'], test_input['did_to_use'], test_input['network'], test_input['private_key_to_use'], 'test/sample.txt')
	if response['status']:
		json_output = json.loads(response['output'])
		for i in json_output['result']:
...
def test_verify_and_show():
	...
	response = hive.verify_and_show(test_input['api_key_to_use'], test_input['did_to_use'],  test_input['network'], request_input)
	if response['status']:
